...
def get_vocab_count_and_level():
    ...
    vocab_count = vocab_df.groupby('Content_ID').size().reset_index(
        name='Vocab_Cal')
    ...
    merged_df['Vocab_Cal'] = merged_df['Vocab_Cal'].fillna(0).astype(int)
    merged_df['Level'] = merged_df['Vocab_Cal'].apply(determine_level)

    final_df = merged_df[['ID', 'Title', 'Link', 'Level', 'Vocab_Cal']]
